...
final class ClassUtilsTest extends FunTestSuite {
  ...
  test("dropping $ from object") {
    ...
    assert(objClazz.getName === "com.chrism.commons.util.ClassUtilsTest$")
    assert(ClassUtils.classNameOf(objClazz) === "com.chrism.commons.util.ClassUtilsTest")
  }

  test("reflectively retrieving Class from its name") {
    val clazz = ClassUtils.fromName("com.chrism.commons.util.ClassUtilsTest")
